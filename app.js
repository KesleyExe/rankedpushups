...
function initPeerConnection() {
  ...
  pc.ontrack = (event) => {
    ...
    if (oppVideo && state.remoteStream) {
      ...
      oppVideo
        .play()
        .catch((e) => console.warn("[WebRTC] Remote video play error:", e));
    }
...
async function createWebRTCOffer() {
  try {
    ...
    // Ensure camera tracks are added
    if (state.cameraStream) {
...
async function handleWebRTCSignal({ signal }) {
  ...
  try {
    if (signal.type === "offer") {
      ...
      // Ensure our camera is attached before answering
      if (state.cameraStream) {
        const senders = pc.getSenders().map((s) => s.track);
        state.cameraStream.getTracks().forEach((track) => {
          if (!senders.includes(track)) {
...
async function startCamera() {
  ...
  try {
    ...
    showError(
      "Camera blocked. You can still play — tap the screen or press spacebar to count reps.",
    );
...
async function initPoseDetector() {
  ...
  try {
    ...
    showError(
      "AI model failed to load. Playing in manual mode — tap screen or press spacebar.",
    );
...
function startPoseDetection() {
  ...
  state.poseDetector.startDetection((data) => {
    ...
    if (phaseEl) {
      phaseEl.textContent =
        data.phase === "down" ? "▼ DOWN" : data.phase === "up" ? "▲ UP" : "--";
    ...
    // Auto-ready detection with retry
    if (
...
function connect() {
  ...
  socket.on("connect", () => {
    ...
    if (name && !state.isRegistered) {
      console.log(
        "[Socket] Registering as",
        name,
        storedElo ? `(ELO: ${storedElo})` : "",
      );
      socket.emit("user:register", {
        name,
        elo: storedElo ? parseInt(storedElo) : undefined,
      });
    }
  });
    ...
    if (state.connectionAttempts >= state.maxConnectionAttempts) {
      showError(
        "Server unreachable after " +
          state.maxConnectionAttempts +
          ' tries. Is "npm start" running?',
      );
    }
  });
  ...
  socket.on("match:ready_check", ({ matchId, opponent, youAre }) => {
    ...
    // Initialize WebRTC for both players early
    initPeerConnection();

    // P1 creates offer after a short delay to ensure P2 is ready
    if (youAre === "p1") {
      setTimeout(() => {
  ...
  socket.on("match:ready_update", ({ p1Ready, p2Ready, p1Name, p2Name }) => {
    ...
    if (youReady && oppReady) {
      if (rs) {
        rs.textContent = "Both players ready!";
        rs.classList.add("both-ready");
      }
      if (brc) {
      ...
      if (rs)
        rs.textContent = state.aiLoaded
          ? "Get into push-up position..."
          : "Click ready when set...";
      if (brc) {
        brc.textContent = state.aiLoaded
  ...
  socket.on("match:start", ({ duration }) => {
    ...
    if (cyr)
      cyr.textContent = state.poseDetector?.repCount || state.manualReps || 0;
  });
  ...
  socket.on(
    ...
    ({
      ...
      if (result === "win") {
        ...
        if (rt)
          rt.textContent = reason === "forfeit" ? "Win by forfeit" : "Victory!";
        if (rs)
          rs.textContent =
            reason === "forfeit"
              ? opponentName + " left"
              : "You out-repped " + opponentName + " by " + Math.abs(diff);
      } else if (result === "loss") {
        if (ri) ri.textContent = "💪";
        ...
        if (rs)
          rs.textContent =
            reason === "forfeit"
              ? "You left"
              : opponentName + " out-repped you by " + Math.abs(diff);
      } else {
        if (ri) ri.textContent = "🤝";
...
function startBotMatch() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      if (rs) {
        rs.textContent = "Both ready! Starting...";
        rs.classList.add("both-ready");
      }
...
function endBotMatch(myReps, botReps, botElo) {
  ...
  // Bot matches NEVER affect ELO - use current stored ELO unchanged
  const currentElo =
    state.user?.elo || parseInt(localStorage.getItem("arena_elo")) || 800;
...
function sendRepsToServer(count) {
  ...
  if (!state.repThrottle) {
    state.repThrottle = setTimeout(() => {
      state.repThrottle = null;
    }, 150);
...
function startQueueAnimation() {
  ...
  queueAnimInterval = setInterval(() => {
    if (state.phase !== "queue") {
      clearInterval(queueAnimInterval);
      return;
    }
    elapsed += 1;
...
function renderLeaderboard(players) {
  ...
  if (!players || !players.length) {
    list.innerHTML =
      '<div class="leader-row"><span style="color:var(--text-tertiary);font-size:13px;">No players yet</span></div>';
    return;
  }
