...
function endBotMatch(myReps, botReps, botElo) {
  ...
  if (result === 'win') {
    ...
    if (rs) rs.textContent = 'You out-repped ' + state.match.opponent.name + ' by ' + Math.abs(diff) + ' (Practice)';
  } else if (result === 'loss') {
    if (ri) ri.textContent = '💪';
    if (rt) rt.textContent = 'Defeat';
    if (rs) rs.textContent = state.match.opponent.name + ' out-repped you by ' + Math.abs(diff) + ' (Practice — no ELO loss)';
  } else {
    if (ri) ri.textContent = '🤝';
    if (rt) rt.textContent = 'Tie!';
    if (rs) rs.textContent = 'Perfectly matched (Practice)';
  }
